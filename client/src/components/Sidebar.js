...
import { useGetBoardsQuery } from "../redux/Slices/api/boardApiSlice";
...
const [selectetedBoardId, setSelectetedBoardId] = useState()
  ...
  useEffect(()=>{
    const getBoards = async()=>{
      try {
        const response = await axios.get(`http://localhost:5000/api/boards/${userName}`)
  ...
  return (
    <div>
      <div
        className={
          isSideBarOpen
            ? `min-w-[261px] bg-white dark:bg-[#2b2c37]  fixed top-[80px] h-screen  items-center left-0 z-20`
                  ...
                  {boards?.map((board, index) => (
                    <div
                      ...
                      onClick={() => {
                        ...
                        setSelectetedBoardId(board._id)
                      }}
                    >
      ...
      {isBoardModalOpen && (
        <AddEditBoardModal
          ...
          selectetedBoardId={selectetedBoardId} 
        />
      )}
    </div>
  );
