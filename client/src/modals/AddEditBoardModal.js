...
import { useCreateBoardMutation, useUpdateBoardMutation } from "../redux/Slices/api/boardApiSlice";

function AddEditBoardModal( { setIsBoardModalOpen ,type,selectetedBoardId } ) {
  ...
  const onSubmit = (type) => {
    ...
    if (type === "add") {
      ...
      console.log("username",user.name,"newColumns",newColumns);
      createBoard({ name, createdBy:user.name, isActive:true, columns:newColumns })
    } else {
      dispatch(boardsSlice.actions.editBoard({ name, newColumns }));
      updateBoard({ name, createdBy:user.name, isActive:true, columns:newColumns },selectetedBoardId)
    }
  };
