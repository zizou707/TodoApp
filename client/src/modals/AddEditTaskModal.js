...
import taskApiSlice, { useAddTaskMutation, useUpdateTaskMutation } from "../redux/Slices/api/taskApiSlice";
...
function AddEditTaskModal({
  ...
  const onSubmit = (type) => {
    if (type === "add") {
      ...
      addTask({  title, description, userId, deadline, status, subtasks, newColIndex, })
    } else {
      dispatch(
      ...
      updateTask({id:taskIndex,title, description, userId, deadline, status, subtasks,taskIndex, prevColIndex, newColIndex,})
    }
  };
    ...
    >
      ...
      >
        ...
        <div className="mt-8 flex flex-col space-y-3">
          ...
          <div className="flex justify-center items-center flex-col h-12 " onClick={()=>setIsDatePickerOpen(!isDatePickerOpen)}>
            <button className=" border">Select a Date</button>
           {deadline && <p> {new Date(deadline).toLocaleDateString()} </p>}
